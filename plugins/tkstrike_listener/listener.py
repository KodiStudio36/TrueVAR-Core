...
from plugins.tkstrike_listener.parser import DaedoStateEngine

class DaedoUDPLifecycleProtocol(asyncio.DatagramProtocol):
    """Low-level AsyncIO datagram socket engine."""
    def __init__(self, parser: DaedoStateEngine, logger, log_callback):
        self.parser = parser
        self.logger = logger
...
class TaekwondoScoreListener(BaseListener):
    ...
    async def connect_to_hardware(self, ip: str, port: int, log_callback=None):
        ...
        try:
            self.transport, self.protocol = await loop.create_datagram_endpoint(
                lambda: DaedoUDPLifecycleProtocol(self.parser, self.logger, log_callback or default_logger),
                local_addr=(ip, port)
            )
